...
// Debug mode - set to true to log detailed OAuth response data
...
serve(async (req: Request) => {
    ...
    try {
        ...
        // Debug: Log token response metadata (excluding actual token values for security)
        if (DEBUG_MODE) {
            console.log(`[${requestId}] DEBUG - ChurchTools token response:`, JSON.stringify({
                ...
                // Log any other metadata fields (but not the actual tokens)
                ...Object.fromEntries(
                    Object.entries(tokenData).filter(([key]) => 
                        !['access_token', 'refresh_token'].includes(key)
                    )
                ),
            }, null, 2));
        }
        ...
        // Debug: Log full userInfo response when DEBUG_MODE is enabled
        if (DEBUG_MODE) {
            console.log(`[${requestId}] DEBUG - ChurchTools userInfo response:`, JSON.stringify(userInfo, null, 2));
        }
